...
第一次使用时，输入邮箱后点“注册”，网站会发送邮箱链接。打开链接登录成功后，在云同步区域设置新密码。设置完成后，这个邮箱以后就可以直接用邮箱和密码登录。
...
打开网站后，在曲谱记录里的云同步区域输入邮箱和密码，点击“密码登录”。第一次使用时，输入邮箱后点击“注册”，打开邮箱链接完成登录，再设置密码。

登录完成后，曲谱会在同一账号的设备间自动合并并保存。已登录状态下默认只显示登录邮箱；需要换密码时点击“修改密码”。
